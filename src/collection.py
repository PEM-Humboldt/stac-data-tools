...
class Collection:
    ...
    def validate_item_against_collection(self, new_item_data, collection_metadata, existing_items):
        """
        ...
        if "metadata" in collection_metadata and "projection" in collection_metadata.get("metadata", {}):
            collection_projection = collection_metadata["metadata"]["projection"].get("epsg")
        
        if collection_projection is None and existing_items:
            # Extract from first existing item
            first_item = existing_items[0]
            collection_projection = first_item.get("properties", {}).get("proj:epsg")
        
        # Get spatial resolution from existing items
        collection_resolution = None
        if existing_items:
            for item in existing_items:
                if "raster:bands" in item.get("assets", {}).get(list(item["assets"].keys())[0], {}):
                    band_info = item["assets"][list(item["assets"].keys())[0]]["raster:bands"][0]
        ...
        if "metadata" in collection_metadata and "data_type" in collection_metadata.get("metadata", {}):
    ...
    def upload_single_item(self, item_data, asset_href=None):
        ...
        try:
            ...
            if asset_href is None:
                ...
                asset_href = f"{settings.asset_base_url}/{settings.abs_container}/{self.stac_collection.id}/{item_data['input_file']}"
            ...
            logger.info(f"Item {item_data['id']} upload response: {item_response.status_code}")
            
            if item_response.status_code in [200, 201]:
                logger.info(f"Item {item_data['id']} uploaded successfully")
                return True
            else:
                logger.error(f"Failed to upload item {item_data['id']}: {item_response.text}")
                return False
